...
class PIDController:
    ...
    def setTime(self, clockTime):
        ...
        if difference > 140 and self.notStopped:
    ...
    def carDetection(self,image):
        ...
        upper = np.array([0,0,79])
        mask = cv2.inRange(hsvCross,lower,upper)

        cutMask = mask[:,0:width/2]
        _,bin = cv2.threshold(cutMask,200,1,cv2.THRESH_BINARY)
        sum = np.sum(bin)
    ...
    def callback(self, data): 
        ...
        if(self.goin and intersection==2):
            if self.carDetection(cv_image):
                self.freeze == True
            else:
                self.freeze == False
